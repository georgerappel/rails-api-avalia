class DisciplinaPeriodoController < ApplicationController
  ...
  def index
    if params[:curso_id]
      @disciplina_periodos = DisciplinaPeriodo.joins('INNER JOIN disciplinas ON disciplinas.codigo = disciplina_periodos.disciplina_codigo').includes(:disciplinas).where('curso = ?', params[:curso_id])
    else
